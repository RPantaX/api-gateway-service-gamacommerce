...
echo "🐳 Docker Build & Push Script para api-gateway Service"
echo "=================================================="
echo ""
...
# Verificar JAR
JAR_PATH="target/api-gateway-0.0.1-SNAPSHOT.jar"
if [ -f "$JAR_PATH" ]; then
...
CONTAINER_ID=$(docker run -d -p 8082:8081 --name api-gateway-service-test "$VERSIONED_TAG")
...
if [ $? -eq 0 ]; then
    ...
    if docker ps | grep -q "api-gateway-service-test"; then
        print_success "Contenedor ejecutándose correctamente"

        # Intentar hacer health check
        print_info "Verificando health check..."
        if docker exec api-gateway-service-test curl -f http://localhost:8081/actuator/health 2>/dev/null; then
        ...
        docker logs --tail 10 api-gateway-service-test

    else
        print_error "El contenedor se detuvo inesperadamente"
        print_info "Logs del contenedor:"
        docker logs api-gateway-service-test
    fi

    # Limpiar contenedor de prueba
    print_info "Limpiando contenedor de prueba..."
    docker stop api-gateway-service-test >/dev/null 2>&1 || true
    docker rm api-gateway-service-test >/dev/null 2>&1 || true
